...
## Реалізовано (as-built, 2026-06-25)

Підсумок фактичної реалізації та уточнень, виявлених під час роботи.

### Нові / змінені файли
- **`infrastructure/citizens/MarionetteInventoryCodec.java`** (новий) — Base64-кодек інвентаря через
  `BukkitObjectOutputStream`/`BukkitObjectInputStream` (зберігає слоти, `null`-порожнечі та PDC/NBT).
  Примітка: `ItemStack.serializeAsBytes()` — Paper-only, у цьому Spigot-API його немає, тож узято
  `ConfigurationSerializable`-шлях через Bukkit-стріми.
- **`infrastructure/citizens/MarionetteMinionTrait.java`** — `save/load(DataKey)` + статичний
  `bindPathwayManager(...)`; шлях зберігається як ім'я (`pathway.getName()`) + рівень.
- **`pathways/fool/abilities/MarionettistControl.java`** — нові публічні методи
  `registerLoadedMarionette(int,UUID)`, `onMarionetteDeath(NPCDeathEvent, IAbilityContext)`,
  `getPossessingCasters()`, `onPluginDisable()`; `cleanUp()` зроблено НЕ руйнівним; прибрано
  `registerNpcDeathListener` і виклик у `convertToMarionette`.
- **`presentation/listeners/MarionetteLifecycleListener.java`** (новий) — `NPCDeathEvent` (смерть) +
  `PlayerQuitEvent` (авто-вихід власника зі збереженням тіла).
- **`presentation/listeners/MarionetteRestorer.java`** (новий) — скан реєстру на
  `CitizensEnableEvent`/`CitizensReloadEvent`.
- **`infrastructure/di/ServiceContainer.java`**, **`MysteriesAbovePlugin.java`** — конструювання/реєстрація
  двох лістенерів, `bindPathwayManager` біля реєстрації трейта, авто-вихід посесій + `onPluginDisable()`
  в `onDisable` ДО `saveAll`.

### Уточнення проти плану
- **Гострий баг, виявлений під час роботи:** `Ability.cleanUp()` викликається на **кожен вихід
  гравця** (через `PassiveAbilityLifecycleListener` → `cleanupPlayer` → цикл по `beyonder.getAbilities()`),
  а `MarionettistControl` — спільний інстанс на пасвей. Старий `cleanUp()` знищував маріонетки **всіх**
  Fool-гравців при виході будь-кого. Тому `cleanUp()` тепер навмисно порожній, а життєвий цикл розведено:
  вихід гравця → `MarionetteLifecycleListener` (PlayerQuitEvent, з контекстом цього гравця);
  вимкнення сервера → авто-вихід у `onDisable` + `onPluginDisable()`.
- **Death-обробка** — замість `onMarionetteDeath(int)` сигнатура `onMarionetteDeath(NPCDeathEvent,
  IAbilityContext ownerCtxOrNull)`: контекст власника передається, лише якщо він онлайн.
- **Юніт-тест регідрації НЕ додано:** `new PathwayManager()` тягне `PotionEffectType` (Bukkit-реєстр)
  під час конструювання абілок і падає headless. Це відповідає конвенції проєкту (`pathways`/ефекти
  верифікуються in-server). Контракт регідрації/кодек/персистентність перевіряються вручну на сервері.
- **Glow після рестарту** вмикається ліниво — щойно власник наступного разу взаємодіє зі здібністю
  (refs захоплюються в `captureRefs`), бо `registerLoadedMarionette` не має контексту кастера.

### Перевірка
- `mvn test` → BUILD SUCCESS, 14/14 (зокрема ArchUnit — межі шарів збережено).
- In-server (ручне): створити маріонетку → рестарт сервера → NPC на місці, інвентар/HP/скін/меню/ліміт
  збереглись, вхід/вихід працює, смерть дропає речі; вихід під час керування + рестарт → авто-swapOut.
