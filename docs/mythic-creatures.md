...
2. Запис у `creatures.yml`: `base_entity`, `tier`, `loot` (тільки інгредієнти), `spawn`
   (`natural` — біоми + заміна ванільного моба; `structure.chance` — вага в лотереї скринь, 0 = ніколи).
...
## Два гейти спавну: місце і сила

Природний, ambient- і структурний спавн проходять **два незалежні** гейти. Плутати їх не можна:
відстань каже, ДЕ починається потойбічне, а тір — КОМУ воно по зубах.

- **Місце** — `creatures.min-spawn-distance` (3000) через `SpawnDistance.isFarEnough`. У Незері
  поріг ділиться на 8 (масштаб координат), тобто там це 375 блоків.
- **Сила** — `domain.creatures.ApexGate.allows(tier, playerSequenceLevel)`. `APEX` (усе, що
  Посл. 5) приходить лише до гравця Посл. ≤6; `COMMON` — до будь-кого. `null` послідовність
  (не потойбічний) apex не отримує ніколи.

`ApexGate` кличе `AmbientCreatureSpawner`. Природний спавн apex не дає взагалі (`replace: []`
→ `pickForBiome` їх не бере). Структурний спавн гейт не кличе, бо виражає те саме правило
вікном послідовностей (див. нижче): Посл. 5 доступна лише гравцеві Посл. 6 або 5. Новий шлях
спавну зобов'язаний пройти той самий гейт; ❌ не дублюй поріг числом у себе.

## Структурний спавн: будь-яка структура, вікно з двох послідовностей

`StructureCreatureSpawnListener` слухає `LootGenerateEvent` — тобто «скриня згенерувала лут».

- **Ключ лут-таблиці не фільтрується.** Село, храм, шахта, бастіон, датапакова чи модова
  структура — усі рівні. Раніше в `creatures.yml` лежали `structure.keys` (`mysteries`,
  `nova_structures`), і без встановленого датапаку ванільні скрині не давали НІКОГО, крім
  блукаючого духа. Поля `keys` більше немає ні в YAML, ні в `SpawnRule`.
- **Чи спавнити** — один конфіг: `creatures.structure.chance` (0.10). Плюс per-chunk cooldown
  5 хв, щоб структура з десятком скринь не дала десяток істот.
- **Кого спавнити** — `CreatureSelector.pickForStructure(playerSequence, bias, roll)`: пул це
  вікно `{seq, seq - 1}` того, хто відкрив. Посл. 9 бачить 9–8, Посл. 8 — 8–7, …, Посл. 6 —
  6–5. **Не-потойбічний рахується Посл. 9**, тож структури працюють і для гравця без шляху.
  Усередині вікна вага — `spawn.structure.chance` самої істоти, помножена на ухил Закону
  Конвергенції (свій шлях гравця важить більше), як в ambient.
- **`structure.chance: 0.0` = зі скринь не приходить.** Так вимкнено вісім істот Світу Духів
  (їх знаходять у Пеклі, `SpiritPact`) і чотирьох `Type: GUARDIAN` — `SafeLocations` дає лише
  сухе місце, а гуардіан на суходолі безпорадний.

Тим самим гейтом фільтрують **HUNT-цілі** `ChurchService.generateTaskSet` і
`SecretOrderService.creatureCandidates(player)` — інакше церква/орден видали б завдання
вполювати істоту, якої гравцеві не покажуть. Фільтр стоїть там, де будується список
кандидатів (є і `CreatureDefinition.tier()`, і гравець), а чисті `*TaskGenerator` про тіри
не знають і знати не мусять.

**Спавн у структурі не має права поставити моба в стіну.** `SafeLocations.spawnableNear`
повертає `Optional`: шукає кільцями від 5 до 10 блоків **від гравця** (не від скрині) тверду
підлогу + 2 блоки простору без рідини; не знайшов — істоти просто не буде. Сусідній
`passableNear` із безумовним фолбеком лишається **тільки для телепорту** вже наявної сутності
(`BlinkBehind`, `Scatter`, `RangedStance`) — для спавну він і був причиною скарг на «моб у
блоках ваншотить зі скрині». Поява ще й телеграфиться звуком і повідомленням за 4 с до
спавну, щоб гравець устиг відступити.
