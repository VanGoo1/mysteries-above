...
`ApexGate` кличуть **обидва** споживачі apex-істот — `StructureCreatureSpawnListener` і
`AmbientCreatureSpawner`. Природний спавн apex не дає взагалі (`replace: []` → `pickForBiome`
їх не бере). Новий шлях спавну зобов'язаний пройти той самий гейт; ❌ не дублюй поріг числом
у себе.
