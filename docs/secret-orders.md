...
- **Схованка ордену** (`domain.organizations.OrderStash`) — невидима членам, `itemKey → n`.
  **Входи**: сід (`seedStashIfAbsent` — при вступі й добивом на гідрації для орденів, у яких
  уже є члени), здача DELIVER-завдань, здача рейдової здобичі
  ...
  інгредієнта рецепта КОЖНОЇ послідовності 8..0 кожного реалізованого
  (`Pathway.hasAnyAbility()`) доступного шляху; ордени «будь-хто» (порожні доступи) — по 1x
  ті самі інгредієнти КОЖНОГО реалізованого шляху в грі (не лише вступника).
  **Посл. 9 сіду не підлягає**: `claimIngredients` завжди просить `getSequenceLevel() - 1`,
  а членство без шляху неможливе, тож затребувана посл. — завжди ≤ 8; сід рівно на 9
  (як було до цього) не міг бути затребуваний ЖОДНИМ гравцем. Ідемпотентність — інертний
  маркер `seed:<шлях>:<посл.>` у самій схованці (той самий прийом, що ключ `recipe:` у
  `ChurchService.seedVaultIfAbsent`), тож повторний виклик добиває лише те, чого бракує.
  **Характеристики сідом НЕ приходять узагалі** — лише рейдовою здобиччю з храму
  (і, за `orders.raid.loot-*`, у пулі вони ЛИШЕ зі свіжими розвідданими).
