...
3. **Нова балансна формула = чистий VO або `domain.services`.** Тест — лише на ЛОГІКУ, що переживає перебалансування (монотонність, межі, інваріанти вибору), а не на конкретні числа: 16 тестів виду «на Посл. 8 вартість дорівнює 30» видалено 11.08.2026, бо вони червоніли на кожному тюнінгу й нічого не захищали. Зразки того, що лишилось: `SequenceScalerTest`, `BrewMatcherTest`, `CreatureSelectorTest`. Запуск: `mvn test -Dtest=SpellRecipeTest` (метод: `-Dtest=SpellRecipeTest#aoeScalesWithPowerAndArea`).
