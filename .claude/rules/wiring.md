...
`stopSchedulers()` → зупинка behavior-ів істот → повернення гравців із маріонеток у власне тіло (див. блок `MarionettistControl` в `onDisable`) → `saveAll()` → `cleanup()` (форс-сейв + `beyonder.cleanUpAbilities()` для кожного — саме тут скасовуються сесії здібностей). Нове, що тримає стан/таски, має влитись у цей ланцюжок.
