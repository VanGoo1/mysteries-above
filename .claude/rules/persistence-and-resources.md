...
Shade-йдуть `glowingentities`, `EffectLib`, `triumph-gui`; `spigot-api` та `coreprotect` — `provided`. Нову бібліотеку додавай свідомо: або в shade (росте JAR), або як plugin-залежність у `plugin.yml`.
