...
- **Мирну версію ворожого моба не роби другим id.** Обнули `Attribute.ATTACK_DAMAGE` при
  ...
  супутник фізично не зможе поранити. Рух при цьому йде не через ціль, а через
  `MobAiCompat.walkTo(...)` (`infrastructure.compat`) — Paper'ів `Mob#getPathfinder()` на
  сервері проєкту падає `NoSuchMethodError`, див. `.claude/rules/minecraft-version.md`.
