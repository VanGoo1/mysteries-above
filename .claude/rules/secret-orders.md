...
- **Вступ — двома шляхами**, обидва йдуть через `SecretOrderService.join`
  ...
  Вступ дає талісман (`OrderItems.createTalisman`) і сідить схованку ордену
  (`seedStashIfAbsent`, якщо ще не сіджена).
...
- **Рейд на сховище храму** (`startRaid` → `RaidSession` → `raidAlarm`/`raidSucceeded`/
  ...
  активної сесії. `RaidSession` — самотіковий об'єкт (власний `BukkitTask`, тік щосекунди),
...
- **Талісман** — головний предмет-меню (`OrderItems.isTalisman`): звичайний ПКМ відкриває
  `OrderMenu.openMain`, sneak-ПКМ стартує рейд на ціль активної RAID-задачі. Перевидача —
  `/order talisman`, кулдаун `orders.talisman.reissue-cooldown-minutes`.
...
- **`order-memberships.json`** (`JSONOrderMembershipRepository`): на гравця —
  ...
  `beyonderKills` (лічильник до наступного запрошення) + кулдаун перевидачі талісмана +
