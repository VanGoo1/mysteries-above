...
- Режим гри (на час OPEN): у залі всі учасники — в `ADVENTURE` (не можуть ламати/ставити
  блоки в світі-заглушці). `open()` запам'ятовує попередній режим у `previousGameModes`
  (in-memory), `restoreGameMode()` повертає його при виході (`close` / `expel` /
  крах-повернення в `handleJoin`). Запис загублено (рестарт сервера під час збору) →
  безпечний дефолт `SURVIVAL`, щоб гравець не лишився замкненим у ADVENTURE. Мапа
  НЕ персиститься (свідомо — гамбіт краху дешевший за схему-міграцію снепшота).
- Виставлення лота / оферта / скупка — предметом у ГОЛОВНІЙ РУЦІ (не GUI-слоти):
  менша поверхня дюпів. Ціни/кількість — через GUI-пікери кнопками +/-
  (`infrastructure.ui.MoneyPicker` — сума `PoundMoney`; `AmountPicker` — ціле в
  `[min,max]`), не чат. Фунти й коппети — окремі контроли пікера (немає
  неоднозначності «15 — фунти чи коппети?»); стан пікера — усього коппетів, живий
  підсумок оновлюється `gui.updateItem`+`update` in-place; є кнопка «Скасувати».
...
- **Введення ціни — повністю через GUI.** Головна рука = товар, який
  віддаєш (лот/оферта). Тип ціни обирається в `MarketMenu.chooseConsideration`
  (міні-меню «Монетами / Предметом»): «Монетами» → `MoneyPicker` (`allowZero=false`,
  бо ціна > 0) → `Consideration.money`; «Предметом (бартер)» → `openBarterPicker`
  (пікер зі знайомих гравцю інгредієнтів `knownIngredientStacks`) → `AmountPicker`
  (кількість 1–64) → `MoneyPicker` (`allowZero=true` — доплата `0` дозволена) →
  `Consideration.of(demand, boot)`. Предметну вимогу можна попросити ЛИШЕ зі
  знайомих інгредієнтів (обмеження пікера). `onCancel` кожного пікера повертає в
  попередній крок (тип ціни / бартер-пікер). Колишній off-hand- і чат-механізми
  прибрано як непомітні для гравця.
